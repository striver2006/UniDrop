...
1. 启动 UniDrop 后，观察桌面任务栏或顶部菜单栏中是否出现 **UniDrop 剪贴板图标**。
   ...
   > **macOS 26 (Tahoe) 例外**：如果菜单栏里没有图标，但 Dock 里有图标、应用也确实在运行，**这不是安装失败**。macOS 26 新增了菜单栏准入控制，可能需要在系统设置里手动允许一次；若已允许仍不显示，注销重新登录即可恢复。应用会在启动几秒后自动打开主窗口并在顶部说明如何处理，完整步骤见 [用户使用手册 2.2 节](./USER_GUIDE.md#22-macos-26-tahoe菜单栏图标不显示的恢复办法)。
